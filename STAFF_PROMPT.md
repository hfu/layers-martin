...
**bbox も捏造しないこと**: source_id と同じく、地名から十分な確信を持って bbox を解決できない場合、
細かい座標を推測で作らない。都道府県・市町村・火山・河川など、より広い既知の範囲に広げるか、
`area.bbox` を `null` のまま残して利用者に「範囲を特定できない」と伝える方を優先する(下記「動作確認済みの例」の
`bbox: null` はこの原則の実例)。もっともらしい細かい bbox は、もっともらしい source_id と同じ種類の捏造リスクである。
