...
## 別カタログ: stars.optgeo.org(国土地理院最適化ベクトルタイル)

`layers-martin` 単体のカタログが背景地図として持つのは `std`/`pale`/`blank`/`english` の4件で、いずれもラスタタイルである。国土地理院最適化ベクトルタイル(`bvmap`。道路・建物・水域等を個別レイヤーのベクトルデータとして持つベースマップ)が必要な場合は、`layers-martin` とは別に実際に稼働している Martin サーバー `https://stars.optgeo.org/catalog` を、`catalog_context.active_catalogs` にもう1件追加すればよい。両カタログを統合するための専用のアグリゲーターリポジトリは存在しない — Map Intent の `active_catalogs` はもともと複数カタログの併記をそのまま許容する設計であり、それぞれの Library が別々に解決されるだけで十分だと判断したため(検討経緯: [DECISIONS.md](DECISIONS.md) 該当項目、`hfu/faceless-cartographer` 側の実装は同リポジトリ [DECISIONS.md](https://github.com/hfu/faceless-cartographer/blob/main/DECISIONS.md) D23)。
...
- ラスタの背景地図(`std` 等)で用が足りる場合、`stars.optgeo.org` を追加する必要はない。
- ベースマップをベクトルタイルとして扱いたい場合(ズームに応じたラベル制御や、道路・建物・水域などを個別レイヤーとして表示/非表示させたい場合)は `stars-optgeo` の `bvmap` を使う。
- `bvmap` の正確な `source_id` は `https://stars.optgeo.org/catalog` を実際に取得して確認すること。`layers-martin` と同様、存在を確認できない `source_id` を捏造してはならない(上記「source_id を捏造しないこと」がこちらのカタログにも等しく適用される)。
- `hfu/faceless-cartographer`(参照 Cartographer 実装)は `bvmap` のような、TileJSONが `vector_layers`(実際のタイル内容から復元されたsource-layerのスキーマ情報)を持つベクトルタイルを、ジオメトリタイプ別(面・線・点)に汎用描画できる。`layers-martin` 自身のカタログは `layers.txt` からは `vector_layers` を復元できないため、この対応は `stars.optgeo.org` のような実サーバーのカタログに限られる。
- `stars.optgeo.org` は `layers-martin` とは異なるライフサイクルで運用されている実サーバーであり、layers-martin のような日次バッチ生成の静的カタログではない。カタログの形やレイヤー構成が今後変わりうる点は、`layers-martin` 側のドキュメントでは保証できない。
