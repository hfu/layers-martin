...
- **2026-08-06**: `STAFF_PROMPT.md`の「正しいやりとりの形」第3項・第5項が`dwg7/spiccato`の設計(URLが一次artifact)と正面から矛盾していた点を解消した([D29](DECISIONS.md#d29-staff_promptmdにspiccato向けの受け渡し方法urlリンク構築手順を追記する))。対象Cartographerで条件分岐する形に差し替え、新設セクション「spiccato 向けURLの構築(`#q=`・`#m=`)」を追加。2026-08-03にscratchpadで下書きされていた提案(`/Users/hfu/spiccato`側セッション由来)を、今回正式に適用した形。`catalog`拡張子無しがバイナリとして扱われうる注意(`.json`付き推奨)も本文に反映済み。
- **2026-08-07**: `dwg7/spiccato`のIssue #1(M365 CopilotによるSTAFF_PROMPT.md/GENNAI_PROMPT.mdの評価レポート)を受け、`STAFF_PROMPT.md`にbbox捏造防止の明記・同名候補の選定手順強化・`name`パラメータの日本語エンコーディング注記・`generated_at`省略可の注記を追加した([D30](DECISIONS.md#d30-dwg7spiccato-issue-1テストレポートの提案を反映する))。GENNAI_PROMPT.md側の対応する変更はspiccato DECISIONS.md D17参照。
- **2026-08-07(同日追記)**: 2点の方針転換。(1) bboxの扱いをnullフォールバック推奨からベストエフォート推測推奨へ逆転(D30追記) — 捏造されたsource_idは対処しようのないエラーを生むが、bboxの粗い推測は利用者が補正できる不便さに留まるため。(2) Staffの応答はUSER(利用者)向けであり、「捏造しません」のような内部規範の遵守表明はUSERには不要という原則を新設([D31](DECISIONS.md#d31-staffの応答はuser向けであり内部規範の遵守を表明する必要はない))。「正直に見つからないと伝える」というUSER向け応答のテンプレートをそのまま含んでいた指示文言を書き換えた。GENNAI_PROMPT.md側の対応する変更はspiccato DECISIONS.md D17追記・D18参照。
- **2026-08-07(さらに追記)**: `dwg7/spiccato`のオープンウェブスタイル(D16)が抱えていた検索ギャップ(日本語の災害名でレイヤーが見つからない)を、`build_catalog.rb`が集約カタログ`catalog.json`に`path`(カテゴリ階層)を含めるようにして解決した([D32](DECISIONS.md#d32-集約カタログcatalogjsonの各エントリにpathを追加するオープンウェブスタイルの検索ギャップ修正))。D16が記録していた対応方向(`description`追加)は調査の結果的外れと判明 — `description`は撮影手法の定型注記のみで災害名を含まないが、`path`には「令和8年(2026年)熊本地震」のように災害イベント名がカテゴリ階層の先頭に入っていた。`dwg7/spiccato`の`searchCatalog`は元々path検索ロジックを持っていたが、集約カタログにpathが無かったため機能していなかった(spiccato側は無改修)。ローカル検証で「熊本地震」検索が0件→45件に改善したことを確認済み。
- **2026-08-13**: `dwg7/spiccato`のIssue #3コメント・Issue #5(`#q=`のlabel構文追加・リンク優先の徹底)を受け、`STAFF_PROMPT.md`の「spiccato 向けURLの構築 (#q=)」節を更新した([D33](DECISIONS.md#d33-dwg7spiccato-issue-3issue-5の指摘を反映する--qのlabel構文リンク優先の徹底))。`req`/`opt`の`source_id|label`構文(半角カンマを含めない注意付き)、コード実行環境があれば`#m=`を試みてから理由付きでYAMLに落ちる段階的フォールバック、リンクを提示できる場合はMap Intentを併記しないことを明記した。
